...
import { ArrowUpRight, ArrowDownRight, BrainCircuit, Activity } from 'lucide-react';
import { cn } from '@/lib/utils';
import { supabase } from '@/lib/supabase';
import { formatDistanceToNow } from 'date-fns';
import { zhCN } from 'date-fns/locale';
import { mockMarketOverview, mockAiInsight, mockHotTopics, mockRecentSignals } from '@/lib/mockData';
...
export function Home() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI Market Summary */}
      <Card className="bg-gradient-to-br from-primary/10 via-surface to-surface border-primary/20 overflow-hidden relative">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs md:text-sm">
              {aiInsight && aiInsight.evidence_items && aiInsight.evidence_items.length > 0 ? aiInsight.evidence_items.map((item, idx) => (
                <div key={idx} className="flex items-center gap-2 text-text-muted">
                  ...
                  <span className="truncate max-w-[150px] md:max-w-none">证据：{item}</span>
                </div>
              )) : (
              ...
              {aiInsight && aiInsight.risk_items && aiInsight.risk_items.length > 0 && aiInsight.risk_items.map((item, idx) => (
                <div key={idx} className="flex items-center gap-2 text-text-muted">
                  <span className="w-1.5 h-1.5 rounded-full bg-down shrink-0"></span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader className="pb-2 md:pb-4">
            <CardTitle className="text-lg md:text-xl">市场概览</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
              {marketOverview.map(market => {
                ...
                return (
                  <div key={market.asset_code} className="p-3 md:p-4 rounded-lg bg-surface-hover border border-border transition-all hover:border-primary/30">
        ...
        <Card className="col-span-1">
          <CardHeader className="pb-2 md:pb-4">
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg md:text-xl">神经脉冲 (Neural Pulse)</CardTitle>
          ...
          <CardContent className="flex flex-col items-center justify-center py-4 md:py-6">
            <div className="relative w-24 h-24 md:w-32 md:h-32 flex items-center justify-center">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" strokeWidth="8" strokeDasharray="283" strokeDashoffset="70" className="text-primary" />
              </svg>
              <div className="absolute flex flex-col items-center">
                <span className="text-2xl md:text-3xl font-bold">75</span>
                <span className="text-[10px] md:text-xs text-text-muted">偏热</span>
              </div>
            </div>
            <div className="mt-4 md:mt-6 w-full space-y-2">
              <div className="flex justify-between text-xs md:text-sm">
                <span className="text-text-muted">情绪分</span>
                <span className="font-mono">82/100</span>
              </div>
              <div className="flex justify-between text-xs md:text-sm">
                <span className="text-text-muted">动量分</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <Card>
          <CardHeader className="pb-2 md:pb-4">
            <CardTitle className="text-lg md:text-xl">今日热点主题</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3 md:space-y-4">
              {hotTopics.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-2.5 md:p-3 rounded-lg hover:bg-surface-hover transition-colors cursor-pointer border border-transparent hover:border-border">
                  <div className="min-w-0 flex-1 mr-3">
                    <div className="font-medium text-sm md:text-base mb-0.5 md:mb-1 truncate">{item.title}</div>
                    <div className="text-xs md:text-sm text-text-muted truncate">{item.summary}</div>
                  </div>
                  <div className="shrink-0">
                    <span className="text-[10px] md:text-xs text-primary bg-primary/10 px-2 py-1 rounded font-medium">热度 {Math.round(item.heat_score || 0)}</span>
                  </div>
                </div>
              ))}
              {hotTopics.length === 0 && (
        ...
        <Card>
          <CardHeader className="pb-2 md:pb-4">
            <CardTitle className="text-lg md:text-xl">最新信号预警</CardTitle>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3 md:space-y-4">
              {recentSignals.map(signal => (
                <li key={signal.id} className="flex gap-3 p-1">
                  <div className={cn(
                    "mt-1.5 w-2 h-2 rounded-full shrink-0 shadow-[0_0_8px_rgba(0,0,0,0.2)]",
                    signal.direction_label === 'bullish' ? 'bg-up' : 
                    signal.direction_label === 'bearish' ? 'bg-down' : 'bg-primary'
                  )} />
                  <div>
                    <p className="text-sm">
                      <span className="font-bold text-primary mr-1">[{signal.signal_title}]</span>
                      {signal.signal_description}
                    </p>
                    <p className="text-xs text-text-muted mt-1">{getRelativeTime(signal.signal_time)}</p>
                  </div>
                </li>
              ))}
              {recentSignals.length === 0 && (
                <div className="text-center py-4 text-text-muted">暂无预警信号</div>
              )}
            </ul>
          </CardContent>
        </Card>
